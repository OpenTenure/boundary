...
public abstract class AbstractBackingBean implements Serializable {
    ...
    protected String successSign = "<span class=\"glyphicon glyphicon-ok form-control-feedback\" style=\"top:0px;\"></span>";
    protected String errorSign = "<span class=\"glyphicon glyphicon-remove form-control-feedback\" data-placement=\"right\" title=\"%s\" rel=\"tooltip\" style=\"top:0px;\"></span>";
    protected String successFunction = "makeDivSuccess";
    protected String errorFunction = "makeDivError";
    protected String validationScript = "<script type=\"text/javascript\">$(function() {%s('%s');$('.form-control-feedback').tooltip()});</script>";

    protected <T extends AbstractModelBean> String validateUI(T bean, String propertyName, String divName, Class<?>... group) {
        String error = bean.getFirstError(bean.validateProperty(propertyName, group));
        return prepareUIValidationResult(error, divName);
    }

    protected String prepareUIValidationResult(String error, String divName) {
        if (!error.equals("")) {
            return String.format(errorSign, error) + String.format(validationScript, errorFunction, divName);
        } else {
            return successSign + String.format(validationScript, successFunction, divName);
        }
    }
    ...
    protected Exception processException(Exception t, String localeCode) {
        ...
        try {
            ...
            if (FaultUtility.hasCause(t, SOLAObjectExistsException.class)) {
                ...
                // Unhandled Exception.
                return new Exception(MessageUtility.getLocalizedMessageText(ServiceMessage.GENERAL_UNEXPECTED, localeCode));
            }
